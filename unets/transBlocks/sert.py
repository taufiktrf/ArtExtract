...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=4, attn_drop=0., qk_scale=None):
        super().__init__()
        self.dim = dim
    ...
    def im2cswin(self, x):
        # print(f"self.num_heads at start of im2cswin: {self.num_heads}")
        B, N, C = x.shape
        H = W = int(np.sqrt(N))
        # print(f"Initial shape: B={B}, N={N}, C={C}, H={H}, W={W}")
        x = x.transpose(-2, -1).contiguous().view(B, C, H, W)
        # print(f"Shape after transpose and view: {x.shape}")

        x = img2windows(x, self.H_sp, self.W_sp)
        # print(f"Shape after img2windows: {x.shape}")

        num_windows = x.shape[0]  # Number of windows
        window_size = self.H_sp * self.W_sp
        # print(f"Number of windows: {num_windows}, window_size: {window_size}")
        # print(self.num_heads)
        x = x.reshape(num_windows, window_size, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        # print(f"Shape after reshape and permute: {x.shape}")
        return x
...
class SSMTDA(nn.Module):
    ...
    def forward(self, x):
        B,C,H,W = x.shape
        # print(f'SSMTDA -> B: {B}, C : {C}, H : {H}, W : {W}')
        x = x.flatten(2).transpose(1, 2)
        # print('SSMTDA - > after flatten : ', x.shape)
        shortcut = x
        x = self.norm1(x)
        # print('SSMTDA - > after normalise : ', x.shape)
        x = x.view(B, H, W, C)
        # print('SSMTDA - > x.view(B, H, W, C): ', x.shape)
        # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x
        # print('SSMTDA - > shifted_x :', shifted_x.shape)
        x_windows = window_partition(shifted_x, self.window_size)  
        # print('SSMTDA - > after window partion x_windows :', x_windows.shape)
...
class SERT(nn.Module):
    ...
    def __init__(self, 
        inp_channel_index=0,
        inp_channels=[512,256,128,64], 
        dim = 8,
        window_sizes=[8,16,16],
        depths=[4,4,4],
        num_heads=[4,4,4],
        split_sizes=[2,4,4],
...
#         weight_factor = 0.1,
        ...
        self.conv_first = nn.Conv2d(inp_channels[inp_channel_index], dim, 3, 1, 1)
        ...
        self.conv_delasta = nn.Conv2d(dim,inp_channels[inp_channel_index], 3, 1, 1)

    def forward(self, inp_img):
        # print('input image shape: ',inp_img.shape)
        _,_,h_inp,w_inp = inp_img.shape
        hb, wb = 16, 16
        pad_h = (hb - h_inp % hb) % hb
        pad_w = (wb - w_inp % wb) % wb
        inp_img = F.pad(inp_img, (0, pad_h, 0, pad_w), 'reflect')
        # print('input after padding : ', inp_img.shape)
        #making error here! check 
        f1 = self.conv_first(inp_img)
        # print('f1 : ',f1.shape)
        x=f1
        for layer in self.layers:
            x = layer(x)
        # print('after for loop: ',x.shape) # torch.Size([8, 8, 64, 64])
        x = self.output(x+f1) #+ inp_img
        # print(x.shape)
        x = self.conv_delasta(x)+inp_img
        # print(x.shape) #torch.Size([8, 512, 64, 64])
        x = x[:,:,:h_inp,:w_inp]
        # print(x.shape)
        return x
